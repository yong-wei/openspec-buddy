...
if ! node -e 'const fs=require("node:fs"); const issue=JSON.parse(fs.readFileSync(process.argv[1],"utf8")); process.exit(String(issue.state || "").toLowerCase() === "open" ? 0 : 1);' "$issue_file"; then
...
if [[ "$clear_lane" == "1" ]]; then
  lane_dir="${OPENSPEC_BUDDY_AUTO_LANE_STATE_DIR:-}"
  if [[ -z "$lane_dir" ]]; then
    lane_dir="$repo_root/openspec/.buddy-cache/auto-lanes"
  fi
  if [[ -d "$lane_dir" ]]; then
    state_key="$(node -e 'const value=process.argv[1] || process.argv[2] || ""; process.stdout.write(String(value).replace(/[^A-Za-z0-9_.-]/g, "-"));' "$current_alias" "$current_path_hash")"
    lane_file="$lane_dir/$state_key.json"
    if [[ -f "$lane_file" ]]; then
    node -e '
...
' "$lane_file" "$issue_number" "$claim_branch"
    fi
  fi
fi
