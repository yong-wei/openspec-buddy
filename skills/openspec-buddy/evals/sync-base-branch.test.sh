...
if [[ "$current_branch" != "integration" ]]; then
  echo "sync-base-branch.sh did not switch to the configured base branch" >&2
  exit 1
fi
...
if [[ "$(git -C "$tmp_dir/work" rev-parse HEAD)" != "$(git -C "$tmp_dir/work" rev-parse origin/integration)" ]]; then
  echo "sync-base-branch.sh did not fast-forward to origin/integration" >&2
  exit 1
fi

printf 'dirty\n' >> "$tmp_dir/work/README.md"
if (cd "$tmp_dir/work" && "$helper") >/tmp/sync-base-dirty.out 2>/tmp/sync-base-dirty.err; then
  echo "sync-base-branch.sh succeeded with a dirty worktree" >&2
  exit 1
fi
if ! grep -F "Worktree must be clean" /tmp/sync-base-dirty.err >/dev/null; then
